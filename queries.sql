...
SELECT vets.name, COALESCE(string_agg(specializations.species_id::TEXT, ', '), 'No Specialization') AS specialties
FROM vets
...
SELECT animals.name AS animal_name, vets.name AS vet_name, visits.visit_date AS date_of_visit
FROM visits
